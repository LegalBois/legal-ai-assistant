...
def get_subcategories(html: str, base_url: str) -> list[str]:
    ...
    for element in child_elements:
        link = element.find("a")
        results.append({"url": urljoin(base_url, link["href"]), "description": link.get_text()})
...
def get_page_items(html: str) -> dict[str, list[str]] | None:
    ...
    if title == "404. Страница не найдена" or main_div.find("ul", class_="results") is None:
        ...
        for item in items:
            ...
            results.append(hash)
        data = {"description": description, "urls": results}
        return data
...
async def fetch(
    ...
    data = {"url": url, "tags": tags, "html": result}
    return data
...
async def fetch_all(url_list, tags_list, proxies, max_concurrent_requests):
    ...
    async with aiohttp.ClientSession(
        timeout=aiohttp.ClientTimeout(total=6), connector=conn
    ) as session:
        tasks = [
            fetch(url, proxy, ua.random, session, semaphore, tags)
...
async def parse_page_items(proxies: cycle, all_data: list, max_concurrency: int, max_workers: int):
    ...
    while len(task_candidates_idx_list) != 0:  # while task_candidates_idx
        ...
        url_list = [
            all_data[task_candidates_idx]["url"] + "?" + urlencode({"page": current_page})
            for task_candidates_idx in task_candidates_idx_list
            if task_candidates_idx is not None
        ]

        pages_raw_data = await fetch_all(
            url_list=url_list,
            tags_list=task_candidates_idx_list,
            proxies=proxies,
        ...
        for task_candidates_idx in task_candidates_idx_list:
            all_data[task_candidates_idx]["tags"][-1] = pages_data[task_candidates_idx][
                "description"
            ]
            all_data[task_candidates_idx]["documents"].extend(
                pages_data[task_candidates_idx]["urls"]
            )
...
async def main(proxies):
    ...
    categories_data = await fetch_all(
        url_list=categories_urls,
        tags_list=categories_tags,
        proxies=proxies,
    ...
    for categ_item in categories_data:
        ...
        subcategories_data = get_subcategories(
            categ_item["html"], categ_item["url"]
        )  # url, description
        all_data.extend(
            [
                {
...
if __name__ == "__main__":
    with open(
        "/mnt/88fdd009-dda3-49d8-9888-cfd9d9d5910a/ITMO/YEAR 2/INTRO_LLM/PROJECT/proxies_new.csv"
    ) as f:
        reader = csv.reader(f)
